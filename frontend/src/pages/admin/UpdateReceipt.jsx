...
function MedicineSearchInput({ value, onSelect }) {
...
// Chuyển 1 dòng detailInfo (từ API) -> item hiển thị trong bảng.
// Không cần id riêng cho dòng: backend xác định "danh tính" 1 dòng dựa trên
// batchId (khóa chính của WarehouseReceiptDetail là (receiptId, batchId)).
function mapDetailToItem(d) {
  const batch = d.batchInfo || {};
  const medicine = batch.medicineInfo || {};
  return {
    key: `batch-${batch.batchId}`,
    medicineId: medicine.medicineId,
    medicineCode: medicine.medicineCode,
    brandName: medicine.brandName,
    unit: medicine.unit || "chưa cập nhật",
    batchNumber: batch.batchNumber,
    productionDate: batch.productionDate || null,
    expiryDate: batch.expiryDate || null,
    importQuantity: d.importQuantity,
    importPrice: Number(d.importPrice),
    isOldBatch: true, // dòng tải từ server luôn gắn với 1 lô đã tồn tại
  };
}
...
export default function UpdateReceipt() {
  ...
  const [editingKey, setEditingKey] = useState(null); // key của dòng đang sửa, null nếu đang thêm dòng mới
  ...
  useEffect(() => {
    ...
    axiosClient
      ...
      .then((res) => {
        ...
        const mappedItems = (receipt.detailInfo ?? []).map(mapDetailToItem);

        setReceiptCode(receipt.receiptCode);
        setReceiptDate((receipt.receiptDate || "").slice(0, 10));
        setSupplierId(String(receipt.supplierId));
      ...
      .catch(() => {
        alertError("Không thể tải thông tin phiếu nhập");
        navigate(-1);
      })
      .finally(() => setLoading(false));
  }, [receiptId, navigate]);
  ...
  // Nạp 1 dòng đã có trong bảng vào form để sửa
  const handleEditItem = (item) => {
    ...
    setRow({
      ...
      medicineLabel: item.brandName || "",
      unit: item.unit || "",
      batchNumber: item.batchNumber || "",
      productionDate: item.productionDate || "",
      expiryDate: item.expiryDate || "",
      quantity: String(item.importQuantity),
      importPrice: String(item.importPrice),
      batchChecked: true,
      isOldBatch: item.isOldBatch,
    });
  };
  ...
  const handleAddItem = async () => {
    ...
    if (editingKey) {
      // Cập nhật dòng đã có trong bảng (backend tự nhận diện dòng cũ/mới qua batchNumber+medicineId)
      setItems((prev) =>
        prev.map((i) =>
          i.key === editingKey
            ? {
  ...
  const handleCancel = async () => {
    ...
    const result = await alertConfirm(
      "Toàn bộ thay đổi chưa lưu sẽ bị hủy, khôi phục về dữ liệu ban đầu.",
      "Hủy các thay đổi?",
      "btn-swal-danger",
    );
    if (result.isConfirmed && snapshot) {
      setReceiptCode(snapshot.receiptCode);
      setReceiptDate(snapshot.receiptDate);
  ...
  const handleSubmit = async () => {
    ...
    setSaving(true);
    try {
      await axiosClient.put(`/warehouse-receipts/${receiptId}`, payload);
      alertSuccess("Cập nhật phiếu nhập thành công");
      navigate(-1);
    } catch (err) {
      const msg = err.response?.data?.message || "Lỗi khi cập nhật phiếu nhập";
  ...
  if (loading) {
    return (
      <>
        <Topbar title="Phiếu nhập Thuốc" subtitle="Đang tải dữ liệu phiếu nhập..." />
  ...
  return (
    <>
      ...
      <div className="p-6 min-h-screen flex flex-col gap-4">
        ...
        {/* Bảng danh sách thuốc trong phiếu */}
                  ...
                  items.map((item) => (
                    <tr
                      ...
                      className={`hover:bg-gray-50 transition text-sm ${
                        editingKey === item.key ? "bg-blue-50/60" : ""
                      }`}
                    >
        ...
        <div className="flex justify-end gap-3">
          <button
            ...
            disabled={saving}
            className="btn-cancel px-8 py-2.5 font-semibold text-sm tracking-wide disabled:opacity-50"
          >
            HỦY THAY ĐỔI
          </button>
          <button
            ...
            disabled={saving}
            className="btn-gradient px-8 py-2.5 font-semibold text-sm tracking-wide disabled:opacity-50"
          >
